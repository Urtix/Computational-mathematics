...
Проведем численный эксперимент, исследующий эффективность (или неэффективность) распараллеливания. Ожидается что коэффициент ускорение будет увеличиваться с размером сетки и он будет положительным.
...
Тогда при уровне доверия 95% значение будет считаться выбрасом если:
...
Было проведено 10 запусков и получены следующие средние значения без выбрасов

Результат каждого из них можно увидеть в Result/res_f2

(Дальше для каждого значения также было проведено по 10 запусков и брались средние значения без выбрасов)
